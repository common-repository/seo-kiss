...
class Meow_SeoKissCore
{
	...
	public $meta_key_skip = '_seokiss_ignore';
	...
	function init() {
		...
		$this->admin = new Meow_SeoKissAdmin( $this );

		// Only for REST
		if ( $this->is_rest ) {
			new Meow_SeoKissRest( $this, $this->admin );
		}

		// Dashboard
		if ( is_admin() ) {
			new Meow_SeoKissUI( $this, $this->admin );
			new Meow_SeoKissSitemap( $this );
		}
    ...
    $messages = [
			...
			"😂 Nice try, human! Even robots like GPTBot get a day off, courtesy of <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>.",
			"🎭 Humans playing robots? What a twist! GPTBot access: Denied by <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>!",
			"👀 Spotted! A human camouflaging as GPTBot. <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a> sees you!",
			"😜 A human mimicking GPTBot? That's some advanced <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a> wizardry!",
			"🤣 Oh, human, GPTBot impressions? You’re fun, but <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a> says no entry!",
			"🤠 Yeehaw! A wild human pretending to be GPTBot? Not on <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>'s watch!",
			"🕵️‍♂️ Detective human, trying to sneak in as GPTBot? <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a> caught ya!",
			"🤪 Human, are you trying to robot? Denied access by the watchful <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>!",
			"🚫 You must be a fun human to mimic GPTBot! But, <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a> keeps the door locked.",
			"😅 Humans doing robot things? Not under <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>’s cyber-watch!",
			"🤨 A human in robot’s clothing! Nice try, but <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a> is not fooled.",
			"🤔 Hmm, human, your GPTBot disguise needs work. Spotted by <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>!",
			"😆 The human is a robot? This plot twist brought to you by <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>!",
			"🧐 An insightful human trying to GPTBot around! But, <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a> is on guard.",
			"🎉 Surprise! Human, you can’t sneak past <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>, even dressed as GPTBot!",
			"🎲 Rolled the dice, human? GPTBot disguise didn’t work on <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>.",
			"🤥 Human, pretending to be GPTBot? <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a> knows your secret!",
			"🧛‍♂️ Ah ah ah, human! Even as GPTBot, you can’t bypass the <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a> barrier!",
			"🤓 Nerdy human alert! GPTBot cosplay isn’t fooling <a href='https://wordpress.org/plugins/seo-kiss/'>AI-SEO KISS</a>!"
    ];
	...
	function reject_gptbot_user_agent() {
		$disallow_gpt_bot = get_option( 'seo_kiss_options' )[ 'seo_kiss_disallow_gpt_bot' ] ?? false;
		if ( $disallow_gpt_bot && isset($_SERVER[ 'HTTP_USER_AGENT' ]) && strpos( $_SERVER[ 'HTTP_USER_AGENT' ], 'GPTBot' ) !== false ) {
			error_log( '[SEO_KISS] GPTBot has been detected and blocked.');
	...
	function check_title_duplicates($title, $id) {
		$duplicate_hashes = get_option( 'seo_kiss_title_hashes' );
		...
		if ( array_key_exists( $title_hash, $duplicate_hashes ) ) {
			if( $duplicate_hashes[ $title_hash ] == $id ) {
				...
				if( empty( $post ) || $post->post_status == 'trash') {
					...
					update_option( 'seo_kiss_title_hashes', $duplicate_hashes );
					return false;
				}
	...
	function calculate_seo_score( $post ) {
		...
		$number_of_tests++;
		$readability_module = new Meow_Modules_SeoKiss_Readability();
		...
		if ( $readability != 0 ) {
			...
			$readability_treshold = get_option( 'seo_kiss_options' )[ 'seo_kiss_readability_treshold' ] ?? 50;
	...
	function google_custom_search( $params ) {
		...
		$cx = get_option( 'seo_kiss_options' )[ 'seo_kiss_google_programmable_search_engine' ] ?? '';
		$key = get_option( 'seo_kiss_options' )[ 'seo_kiss_google_api_key' ] ?? '';
		$search_depth = (int)get_option( 'seo_kiss_options' )[ 'seo_kiss_google_search_depth' ] ?? 3;

		if ( empty( $key ) ) { error_log( '[SEO_KISS] Google API Key is missing.' ); return false; }
		if ( empty( $cx ) ) { error_log( '[SEO_KISS] Google Programmable Search Engine ID is missing.' ); return false; }
		...
		for ($i = 0; $i < $search_depth; $i++) {
			...
			if ( is_wp_error( $response ) ) {
				error_log( '[SEO_KISS] Request to Google Custom Search API failed: ' . $response->get_error_message() );
				return false;
			}
	...
	function magic_fix( $post, $errors, $update_post = false){
		...
		if (is_null( $mwai ) || !isset( $mwai ) ) {
			error_log( '[SEO_KISS_MAGIC_FIX] Missing AI Engine.');
			return false;
		}

		$magic_fix_module = new Meow_Modules_SeoKiss_Magic_Fix( $post );
		
		$fix_to_ignore = get_option( 'seo_kiss_options' )[ 'seo_kiss_select_magic_fix' ] ?? [ ];
		$errors = array_diff( $errors, $fix_to_ignore );

		foreach ( $errors as $error_key => $error) {
			$error_switch = $update_post ? $error_key : $error;
			$elapsed_time = microtime( true ) - $start_time;
			error_log( '[SEO_KISS_MAGIC_FIX] Elapsed time : ' . sprintf('%0.2f', $elapsed_time) . ' seconds. Now processing : ' . $error_switch );
			switch ( $error_switch ) {
	...
	function can_access_settings() {
		return apply_filters( 'seo_kiss_allow_setup', current_user_can( 'manage_options' ) );
	}

	function can_access_features() {
		return apply_filters( 'seo_kiss_allow_usage', current_user_can( 'administrator' ) );
	}
	...
	function refresh_options(){
		// seo_kiss_posts_types and seo_kiss_language should be refreshed as they are dynamic
		$options = $this->get_all_options();
		
		$options['seo_kiss_posts_types'] = $this->make_post_type_list( $this->get_post_types() );

		if ( $options['seo_kiss_language'] == 'auto' || empty( $options['seo_kiss_language'] ) ) {
			$options['seo_kiss_language'] = $this->get_language_name( get_locale() ) ?? 'English';
		}
	...
	function list_options() {
		return array(
			  //POST TYPEX
			'seo_kiss_default_post_type' => 'post',
			'seo_kiss_posts_limit' => 10,
			'seo_kiss_posts_types' => $this->make_post_type_list( $this->get_post_types() ),
			'seo_kiss_select_post_types' => ['post'],

			//PREFERENCES
			'seo_kiss_readability_treshold' => 50,
			//get_locale to full language name
			'seo_kiss_language' => $this->get_language_name( get_locale() ) ?? 'English',
			'seo_kiss_disallow_gpt_bot' => false,

			//RANKING
			'seo_kiss_google_ranking' => false,
			'seo_kiss_google_api_key' => '',
			'seo_kiss_google_programmable_search_engine' => '',
			'seo_kiss_google_interval_hours' => 24,
			'seo_kiss_google_search_depth' => 3,
			'seo_kiss_google_track_points' => 60,

			//AI PARAMETERS
			'seo_kiss_ai_magic_fix' => false,
			'seo_kiss_ai_auto_correct' => false,
			'seo_kiss_select_magic_fix' => [
			...
			'seo_kiss_ai_magic_wand' => false,
			'seo_kiss_ai_web_scraping' => false,
			'seo_kiss_ai_keywords' => false,
		);
	}
