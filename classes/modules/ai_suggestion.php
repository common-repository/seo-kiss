...
class Meow_Modules_SeoKiss_Ai_Suggestion
{
    public static function prompt($post, $field, $customContent = null, $max_tokens = 150, $meta_key_seo_title = '_kiss_seo_title', $meta_key_seo_excerpt = '_kiss_seo_excerpt' ){
		...
		$language = get_option( 'seo_kiss_options', null )[ 'seo_kiss_language' ] ?? 'English';
		$ai_keywords = get_option( 'seo_kiss_options', null )[ 'seo_kiss_ai_keywords' ] ?? false;

		$originalContent = $customContent?? Meow_Modules_SeoKiss_Ai_Suggestion::get_post_sample_context( $post );
		$prompt = "";
		...
		$ai_suggestion = Meow_Modules_SeoKiss_Ai_Suggestion::verify_ai_suggestion( $ai_suggestion, $expected_size, $language );
	...
	public static function verify_ai_suggestion( $ai_suggestion, $expected_size, $language ) {
		...
		$is_auto_correct_enabled = get_option( 'seo_kiss_options', null )[ 'seo_kiss_ai_auto_correct' ] ?? false;
