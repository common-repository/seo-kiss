...
class Meow_SeoKissRest
{
	private $core = null;
	private $namespace = 'seo-kiss/v1';
	...
	function rest_settings_update( $request ) {
		try {
			...
			$success = !!$options;
			$message = __( $success ? 'OK' : "Could not update options.", 'seo-kiss' );
	...
	function rest_settings_reset() {
		try {
			...
			$success = !!$options;
			$message = __( $success ? 'OK' : "Could not reset options.", 'seo-kiss' );
			return new WP_REST_Response([ 'success' => $success, 'message' => $message, 'options' => $options ], 200 );
		}
		catch ( Exception $e ) {
			$message = apply_filters( 'mwai_ai_exception', $e->getMessage() );
	...
	function rest_generate_sitemap() {
		try {
			$sitemap = new Meow_SeoKissSitemap( null );
	...
	function createValidationResult( $result = true, $message = null) {
		$message = $message ? $message : __( 'OK', 'seo-kiss' );
	...
	function rest_posts( $request ) {
		$post_type = get_option( 'seo_kiss_options', null)['seo_kiss_default_post_type'] ?? null;
	...
	function rest_update_post( $request ) {
		...
		$this->update_or_delete_post_meta( $post_id, '_seo_kiss_ai_keywords', $ai_keywords );
	...
	function rest_fetch_searches(  ) {
		$searches = get_option( 'seo_kiss_searches', [] );
		$interval = get_option( 'seo_kiss_options', null )[ 'seo_kiss_google_interval_hours' ] ?? 24;
		$track_points = (int)get_option( 'seo_kiss_options', null )[ 'seo_kiss_google_track_points' ] ?? 60;
		...
		update_option( 'seo_kiss_searches', $searches );
	...
	function rest_ai_suggest( $request ) {
		try {
			...
			if (is_null( $mwai ) || !isset( $mwai ) ) {
				...
				$ai_suggestion = Meow_Modules_SeoKiss_Ai_Suggestion::prompt( $post, $params[ 'field' ] );
			}
	...
	function rest_ai_web_scraping( $request ) {
		try {
			...
			$ai_suggestion = Meow_Modules_SeoKiss_Scraping::scrap_bing_request( $search );
